...
void UAuraAttributeSet::PreAttributeChange(const FGameplayAttribute& Attribute, float& NewValue)
{
	...
	//Clamping any attributes here. Health and Mana for now.
...
void UAuraAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if (Data.EvaluatedData.Attribute == GetHealthAttribute())
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("Changed health on %s, Health: %f"), *OutEffectProps.TargetAvatarActor->GetName(), GetHealth());
	}
	...
	if (Data.EvaluatedData.Attribute == GetIncDamageAttribute())
	{
		const float LocalIncDamage = GetIncDamage();
		SetIncDamage(0.f);

		if (LocalIncDamage > 0.f)
		{
			const float NewHealth = GetHealth() - LocalIncDamage;
...
const void UAuraAttributeSet::SetEffectProperties(const FGameplayEffectModCallbackData& Data, FEffectProperties& OutEffectProps)
{
	...
	if (IsValid(OutEffectProps.SourceASC) && OutEffectProps.SourceASC->AbilityActorInfo.IsValid() && OutEffectProps.SourceASC->AbilityActorInfo->AvatarActor.IsValid())
	{
		...
		//In case it was a pawn and not a character
